...
logo_path = resource_path("Cadence_Logo_2Rev_185_Reg-298x56-0f8f5aa.png")
# open method to open image
...
def run_composite_extractor(log_file):
    ...
    non_page_pattern = re.compile(
        "\d+-\d+\s\d+:\d+:\d+.*\sdetected\shigh\snon-page\smemory.*"
    )
    flag_panicking = 0
    ...
    with open(log_file, "r", encoding="latin1") as lines:
        for line in lines:
            if re.search(starter_pattern, line) is not None:
                started_status = re.search(r"(\d+-\d+\s\d+:\d+:\d+)\s[0-9]+\.[0-9]+s.*Worker\s(\d+)\sstarted.*",
                    line,)
        ...
        merged_lists = list(
            map(lambda x, y: (x, y), time_worker_stamps_list, composites_list)
        )
        merged_high_non_page_dict = dict(
            list(
                map(
                    lambda x, y: (x, y),
                    high_non_page_time_worker_stamps_list,
        ...
        df_composites = pd.DataFrame(
            {key: pd.Series(value) for key, value in dict_of_merged.items()}
        )

        df_worker_started = pd.DataFrame(
            worker_started_list, columns=["TimeID", "WorkerID"]
        )
        # method to get a list of unique composites, avoid repitition
        df_high_non_page_composites = pd.DataFrame(
            {key: pd.Series(value) for key, value in merged_high_non_page_dict.items()}
        )
        if df_high_non_page_composites.empty:
            worker_started = str(df_worker_started["TimeID"][0])
            df_high_non_page_composites = pd.DataFrame(
                {worker_started + ", -5000": ["Nan"]}
            )
        ...
        def flatten_unique_list(extracted_list):
            set_comps = {
                i
                for b in [
                    [i] if not isinstance(i, list) else flatten_unique_list(i)
        ...
        return (
            df_composites,
            extracted_unique_list,
            df_worker_started,
            df_high_non_page_composites,
            extracted_unique_high_non_page_list,
        )
        ...
        for line in lines:
            if re.search(status_pattern, line) is not None:
                ...
                match_status = re.search(
                    r".*\s\d+-(\d+-\d+\s\d+:\d+:\d+)\s*Elapsed:\s+\d+:\d+:\d+", line)
                ...
                or re.search(warehouse_pattern, line) is not None)
                and (flag_status == 1)
                and (flag_worker == 1)
            ):
...
def heartbeat_extractor(comp_log, filename):
    heartbeat_pattern = re.compile(".*:\sHeartbeat:.*")
    cluster_data_pattern = re.compile(".*Warning:\sLarge\scluster\sData\sTransfer.*")
    computer_pattern = re.search(r"computer-(\d+).log", filename)
    computer_id = int(computer_pattern.group(1))
    non_paged_mem_list = []
    warning_list = []

    with open(comp_log, "r", encoding="latin1") as lines:
        for line in lines:
            if re.search(heartbeat_pattern, line) is not None:
                heartbeat_status = re.search(
                    r"(\d+-\d+\s\d+:\d+:\d+)\s[0-9]+\.[0-9]s.*Heartbeat:.*mem_mb=(\d+)\spage_peak_mb=(\d*).*",
                    line,
                )
                time_stamp = datetime.strptime(
                    heartbeat_status.group(1), "%m-%d %H:%M:%S"
                )
                ...
                heartbeat_dict = {
                    ...
                    "TimeID": time_stamp,
                    "Non_Paged_Mem_MB": non_paged_mem,
                }
                non_paged_mem_list.append(heartbeat_dict)
            elif re.search(cluster_data_pattern, line) is not None:
                cluster_data_status = re.search(
                    r"^(\d+-\d+\s\d+:\d+:\d+)\s[0-9]+\.[0-9]+s.*Warning:\sLarge\scluster\sData\sTransfer\s(.*)",
                    line,
                )
                time_stamp = datetime.strptime(
                    cluster_data_status.group(1), "%m-%d %H:%M:%S"
                )
        ...
        df_non_paged_mem_mb = pd.DataFrame(
            non_paged_mem_list, columns=["ComputerID", "TimeID", "Non_Paged_Mem_MB"]
        )
        df_warning = pd.DataFrame(
            warning_list, columns=["ComputerID", "TimeID", "WarningID"]
        )
    ...
    if check_dir_exist(directory):
        ...
        for file in os.listdir(directory):
            ...
            if filename.startswith("computer") and filename.endswith(".log"):
                df_non_paged_mem, df_warning = heartbeat_extractor(
                    reports_input + "/" + filename, filename
                )
        ...
        df_non_paged_mem = pd.DataFrame()
        df_cluster_warning = pd.DataFrame()
...
def create_data(input_log_location):
    ...
    subprocess.run(
        "/vols/ssvrd_t1_007/meg/commander/workspace/hourly_opt/gcc_v9/latest/output/cdn_hier/lnx86_OPT_gcc_v9/tools.lnx86/bin/pegasus -decrypt "
        + input_log_location
        + "/pegasus.log "
        + output_directory,
        shell=True,
    ...
    (
        df_composites,
        extracted_composites_list,
        df_worker_started,
        df_high_non_page_composites,
        extracted_unique_high_non_page_list,
    ) = run_composite_extractor(decrypted_location)
    ...
    print('#########################################')
    string_list = (
        string_list.replace(" ", "").replace("'", "").replace("[", "").replace("]", "")
    )
    print("Generating Rules Report...")
    subprocess.run(
        "/lan/csv/geomrd1/tools/bin/generate_stats.sh.b71746 --force --run-dir "
        + results_dir
        + "/fanout --log-dir "
        + input_log_location
        + "/ --pegasus /vols/ssvrd_t1_007/meg/commander/workspace/hourly_opt/gcc_v9/latest/output/cdn_hier/lnx86_OPT_gcc_v9/tools.lnx86/bin/pegasus --skip-reports --report-fanout-rules "
        + string_list,
        shell=True,
    )
    # get stats.rpt.fanout file
    rules_file_input = results_dir + "/fanout/stats.rpt.fanout"
    if os.path.isfile(rules_file_input):
        ...
        df_rules = pd.DataFrame(columns=["CompositeID", "EngineName", "Rules"])
        data_list = []
        data_list.append(
            {
                "CompositeID": "No Panics reported",
                "EngineName": "No Panics reported",
                "Rules": "No Panics reported",
            }
        )
        df_rules = pd.DataFrame.from_records(data_list)
        print("stats.rpt.fanout rules file does not exist...")
    ...
    (
        ...
        df_worker_finished,
        df_worker_killed,
    ) = run_usr_log_extractor(input_log_location + "/usr.log")
    ...
    df_high_non_page_composites.to_csv(
        db_dir + "/df_high_non_page_composites.csv", index=False
    )
...
if logs_input.endswith('/'):
    logs_input = logs_input[:-1]
...
try:
    ...
    df_composites = pd.DataFrame()#empty database in case of no panics reported
try:
...
#get graph size for height based on num of workers
def get_graph_size(max_worker):
    if max_worker <= 5:
        mem_graph_size = 5 
        heatmap_size = max_worker
        max_height = 500 + (max_worker * 100)
    else:
        heatmap_size = 1
        max_height = 1000
        mem_graph_size = 1
    return heatmap_size, max_height, mem_graph_size

heatmap_size, max_height, mem_graph_size = get_graph_size(max_worker)
# 5 trace graphs
fig = make_subplots(
    rows=2,
    cols=1,
    ...
    row_heights=[mem_graph_size, heatmap_size],
...
if df_outer_merge.empty == False:
    trace1 = go.Histogram2d(
        ...
        hovertemplate="<i>Time: <i> %{x}" + "<br><b>Worker:</b>: %{y}<br>",
    )
...
fig.update_layout(
    ...
    legend=dict(orientation="v", y=0.9, x=1),
)
...
for cpu in cpus:
    fig2.add_trace(
        go.Scatter(
            name="Computer-" + str(cpu),
            x=df_non_paged_mem[df_non_paged_mem["ComputerID"] == cpu]["TimeID"],
            y=df_non_paged_mem[df_non_paged_mem["ComputerID"] == cpu][
                "Non_Paged_Mem_MB"
            ],
            mode="lines",
            hovertemplate="<i>Time: <i> %{x}"
    ...
    for cpu in cpus:
        fig3.add_trace(
            go.Scatter(
                ...
                y=df_cluster_warning[df_cluster_warning["ComputerID"] == cpu][
                    "ComputerID"
                ],
                mode="markers",
                marker=dict(
...
text_style = {"textAlign": "center", "color": colors["text"]}
...
app.layout = html.Div(
    ...
    children=[
        html.Img(src= logo, style= logo_style),
        dcc.Tabs(
            children=[
                # tab for heatmap
                dcc.Tab(
                    ...
                    children=[
                        html.Div(
                        html.H1(children="Panic Heatmap", style=text_style),
                        ),
                        html.Div(dcc.Graph(id="graph", figure=fig, style = {'height': max_height})),
                        html.Div(
                            [
                                dcc.Markdown(
                                    "Click on x points in the graph to show details.",
                                    style=text_style,
                                ),
                                html.Pre(id="click-data"),
                                html.P(id="time_worker_details", style=text_style),
                                html.Div(
                                    id="table1_div",
                                    children=[
                ...
                dcc.Tab(
                    label="Non Paged Memory",
                    children=[
                        html.H1(children="Non Paged Memory", style=text_style),
                        html.Div(
                            id="tab_container",
                            children=[
                                html.Div(
                                    id="non_paged_graph_container",
                                    children=[
                                        dcc.Graph(id="non_paged_graph", figure=fig2),
                                        html.P(
                                            id="time_worker_details2", style=text_style
                                        ),
                                        html.Div(
                                            id="table2_div",
                                            children=[
                                                ...
                                                html.Button(
                                                    "Export to PVL",
                                                    id="non_paged_mem_button",
                                                    n_clicks=0,
                                                    style=button_style,
                                                ),
                                                html.Table(
                                                    id="non_paged_composite_table"
                                                ),
                                ...
                                html.Div(
                                    id="cluster_data_graph_container",
                                    children=[
                                        html.H2(
                                            children="Cluster Data Warnings",
                                            style=text_style,
                                        ),
                                        dcc.Graph(
                                            id="cluster_data_figure", figure=fig3
                                        ),
                                    ],
                                ),
                                html.P(id="time_worker_details3", style=text_style),
                                html.Div(
                                    id="table3_div",
                                    children=[
...
def Update_data_table(clickData):
    if clickData:
        ...
        xy = x + ", " + y
        if xy in df_composites.columns:
            ...
            if df_rules.empty:
                df[''] = 'No data generated in stats.rpt.fanout'
                df_sub = df[xy]
                df.rename(columns={ df.columns[0]: "CompositeID" }, inplace = True)
                df_sub_table = dash_table.DataTable(
                    df.to_dict("records"),  # return as a table
                ...
                df_sub = df_rules["CompositeID"].isin(
                    df[xy]
                )  # filter selected compositeId by selected
                df_sub_table = dash_table.DataTable(
                    df_rules[df_sub]
                    .drop_duplicates(subset=["CompositeID"])
                    .to_dict("records"),  # return as a table
                    style_cell={"textAlign": "left"},
                    style_header={"backgroundColor": "rgb(30, 30, 30)", "color": "white"},
                    style_data={"backgroundColor": "rgb(50, 50, 50)", "color": "white"},
                )
...
def update_non_paged_table(clickData):
    if clickData:
        ...
        y = str(click_details_dict["y"])
        xy = x + ", " + y
        if xy in df_high_non_page_composites.columns:
            xy_string = x[5:]
            df = df_high_non_page_composites[[xy]]
            if df_rules.empty:
                # df = df.assign(EmptyCol= 'No data generated in stats.fanout.rpt')
                df[''] = 'No data generated in stats.rpt.fanout'
                df_sub = df[xy]
                df.rename(columns={ df.columns[0]: "CompositeID" }, inplace = True)
                df_sub_table = dash_table.DataTable(
...
def update_warning_table(clickData):
    if clickData:
        ...
        df_sub = pd.DataFrame(
            df_cluster_warning.loc[
                df_cluster_warning["TimeID"].eq(x)
                & df_cluster_warning["ComputerID"].eq(y),
                "WarningID",
            ]
        )
        df_sub_table = dash_table.DataTable(
            df_sub.to_dict("records"),
            style_cell={"textAlign": "left"},
            style_header={"backgroundColor": "rgb(30, 30, 30)", "color": "white"},
            style_data={"backgroundColor": "rgb(50, 50, 50)", "color": "white"},
        )
